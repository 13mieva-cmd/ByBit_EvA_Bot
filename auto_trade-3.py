    ...
    async def handle_signal(self, signal: dict):
        ...
        async with self._signal_lock:
            ...
            if TRADE_TIME_FILTER_ENABLED:
                ...
                if in_block:
                    log.info(f"time filter UTC {hour}:00 skip {signal.get('symbol')}")
                    return
            ...
            try:
                ...
                self.state._save()
            tp_disp = (
                f"<code>${result['tp_price']:.6g}</code> (+{tp_pct}%)"
                if result.get("tp_price") is not None
                else f"soft/trail (+{tp_pct}%)"
            )
            sl_disp = result.get("sl_price") or 0
            await self.notify(
                f"✅ <b>{base}</b> открыта ({sig_type})\n\n"
                f"Вход: <code>${pos['entry_price']:.6g}</code>\n"
                f"Размер: ${pos_usd:.0f} · {result['leverage']:.0f}x\n"
                f"🎯 TP: {tp_disp}\n"
                f"🛑 SL: <code>${sl_disp:.6g}</code> (−{sl_pct}%)\n"
                f"Позиций: {len(self.state.active_positions)}/{MAX_AUTO_POSITIONS}"
            )
