...
НАЙДЕНО ПО РЕАЛЬНЫМ СКРИНАМ ИЗ TELEGRAM (не гипотеза, а факт использования):
- пользователь нажимал "Я вошёл" на карточках card_early() и card_reversal() —
  ТЕХ САМЫХ, что своим же текстом трижды повторяют "НЕ подтверждённый сетап",
  "пробой вверх и вниз почти равновероятны", "размер пробный". Кнопка входа при
  этом была ВИЗУАЛЬНО НЕОТЛИЧИМА от кнопки на подтверждённом long-сигнале —
  тот же текст "✅ Я вошёл" везде. "Плохие точки входа" из предыдущего разбора —
  это вход на 50/50 ставке по сжатию (входишь ровно на границе диапазона, что и
  выглядит как "у максимума", если пробой не удержался), а не баг в spike_guard,
  который на early/reversal и не распространяется (сознательно, см. выше).
- баг НЕ в математике — в UI: кнопка card_early теперь "✅ Пробный вход (не
  подтверждено)", card_reversal — "✅ Пробный вход (риск повышен)". card_early15
  кнопки не имеет вообще (уже было сделано верно до этого прохода).
- добавлено поле tier ("trial"/"confirmed") в POSITIONS, проставляется из 4-го
  поля callback_data (обратная совместимость: старые кнопки с 3 частями по
  умолчанию считаются confirmed, не падают). Виден в /pos (значок 🔶) и в каждом
  сообщении position_status() на всём протяжении сделки — не только в момент
  нажатия кнопки, чтобы пометка не терялась через час после входа.

НОВЫЙ ДЕТЕКТОР: 15м-версия пробоя сжатия (разбор реального случая — 1000PEPE,
скрин в чате: узкое сжатие на 4-5 пятнадцатиминутках, потом +6.8% на ОДНОЙ
свече; посчитано по калиброванной ценовой оси скриншота, не на глаз):
- detect_compression() параметризована аргументом zone_bars (по умолчанию 24 —
  ровно как было; часовой вызов не тронут). Первая попытка переиспользовать
  функцию как есть на 15м барах не сработала: окно "текущего сжатия" в 24 бара
  на 15м — это 6 часов, а не тот короткий (~1-1.5ч) эпизод, что виден на
  скриншоте. Поймано на синтетике, повторяющей форму PEPE (0/1 срабатывание с
  zone_bars=24 против ловится с zone_bars=6) — до передачи в прод.
- добавлен параллельный 15м-детектор (EARLY_C15_ENABLED, отдельный кулдаун
  LAST_EARLY_C15, отдельная статистика "early_c15") поверх УЖЕ загруженных 15м-
  баров (те же, что для detect_early_15m/треугольника) — новых запросов к API
  не требует. Использует ту же card_early() с меткой "15м" в заголовке и
  отдельным треком в /stats, чтобы было видно, отличается ли качество короткого
  детектора от часового, а не смешивать статистику двух разных по духу вещей.
- честно: для движений, которые целиком укладываются в 1-2 свечи (сам PEPE-спайк
  — это буквально одна 15м-свеча), физический потолок "раньше" — это момент
  пробоя зоны сжатия, а не более ранняя точка: до пробоя направление неизвестно
  по определению (см. текст самой карточки: "пробой вверх и вниз почти
  равновероятны"). Это ловит сжатие раньше, чем часовой детектор его увидит —
  не гарантирует бота "у самого дна" для любого движения.
...
# 15м-версия того же детектора сжатия (переиспользует detect_compression на 15м барах,
# которые и так уже тянутся для detect_early_15m/треугольника — доп. запросов к API не нужно).
# Ловит компрессии длительностью ~1-2ч, которые часовой detect_compression() физически не
# видит: тому нужно ~24ч+ часового контекста, чтобы вообще опознать "зону".
EARLY_C15_ENABLED=True
EARLY_C15_COOLDOWN_H=2
LAST_EARLY_C15={}           # coin -> ts последнего 15м-сжатия (отдельно от часового)
...
def detect_compression(highs, lows, closes, vols, zone_bars=24):
    ...
    zone_bars — ширина окна "текущего сжатия" в барах (по умолчанию 24 — это и
    было единственным вариантом раньше; параметризовано, чтобы то же самое можно
    было прогнать на 15м барах с окном покороче — 24 15м-бара это 6ч, а не то
    же самое понятие "сжатие", что 24 часовых бара).
    Возвращает (just_broke_up: bool, zone_hi, zone_lo)."""
    n=len(closes)
    if n<max(60, zone_bars+36): return False,0,0
    # диапазон последних zone_bars свечей БЕЗ самой последней (окно сжатия)
    win_h=highs[-zone_bars-1:-1]; win_l=lows[-zone_bars-1:-1]
    zone_hi=max(win_h); zone_lo=min(win_l)
    cur_range=zone_hi-zone_lo
    if cur_range<=0: return False,0,0
    # исторические диапазоны за предыдущие 30 окон по zone_bars свечей (НЕ пересекаются с текущей зоной)
    hist=[]
    for i in range(zone_bars,zone_bars+30):
        e=n-1-i; s=e-zone_bars
        if s<0: break
    ...
    vb=sum(vols[-zone_bars-1:-1])/zone_bars if len(vols)>=zone_bars+1 else (sum(vols)/len(vols))
...
def card_early(m, zone_hi, zone_lo, tf_label="", sig_type="early"):
    ...
    lines=[
        f"\U0001F535 <b>{m['coin']} · РАННИЙ СИГНАЛ{tf_label}</b> (эксперим.)",
    ...
    _n,_w=track_record(sig_type)
    if _n>0:
        lines += ["", f"\U0001F4C8 Трек-рекорд РАННИХ{tf_label}: измерено {_n}, в плюсе через 24ч {_w} ({_w/_n*100:.0f}%)"]
    lines += ["", "\u2501"*16,
...
def compute_stats():
    ...
    for horizon_h, label in [(4,"4ч"),(24,"24ч")]:
        for sig_type in ("long","triangle","early","early_c15","early15","reversal"):
            sig_pcts=[]; btc_pcts=[]
            for r in rows:
...
def position_status(coin):
    ...
    tier_tag="\U0001F536 [ПРОБНЫЙ] " if p.get("tier")=="trial" else ""
    ...
    if reasons:
        msg=(f"{tier_tag}\U0001F6A8 {coin}: похоже на РАЗВОРОТ — пора решать!\n"
    ...
    if r_mult<=-DRAWDOWN_FLAG_R:
        msg=(f"{tier_tag}\U0001F7E0 {coin}: минус {abs(pnl)*100:.1f}% (\u2248{r_mult:.1f}R от риска на входе)\n"
    ...
    if r>=RSI_TAKE_PARTIAL and pnl>0:
        msg=(f"{tier_tag}\U0001F7E1 {coin}: RSI разогнался до {r:.0f} при прибыли {pnl*100:+.2f}%\n"
    ...
    if r_mult>=TRAIL_TRIGGER_R:
        msg=(f"{tier_tag}\U0001F4C8 {coin}: в плюсе {pnl*100:+.2f}% (\u2248{r_mult:.1f}R от риска на входе)\n"
    ...
    if opened_h>=STALL_HOURS and abs(pnl)<STALL_BAND:
        msg=(f"{tier_tag}\U0001F634 {coin}: открыта уже {opened_h:.0f}ч, P&L почти не сдвинулся ({pnl*100:+.2f}%)\n"
    ...
    msg=(f"{tier_tag}\U0001F7E2 {coin}: держится\n"
        f"P&L: {pnl*100:+.2f}% (вход ${p['entry']:.5g} → ${price:.5g})\n"
        ...
        if EARLY_ENABLED:
            ...
            if (broke and m.get("rsi",100)<=EARLY_RSI_MAX and m["dd"]>KNIFE_DD
                ...
                if now-le>=EARLY_COOLDOWN_H*3600:
                    LAST_EARLY[coin]=now
                    bt=[[{"text":"\u2705 Пробный вход (не подтверждено)","callback_data":f"enter|{coin}|{m['price']:.6g}|trial"}]]
                    tg_send(cid, card_early(m,zhi,zlo), buttons=bt); shown+=1
                    log_signal(coin, "early", m["price"])

        # === 15м-ВЕРСИЯ того же детектора сжатия: короткие (~1-2ч) компрессии, которые
        # часовой детектор выше физически не видит (см. коммент у EARLY_C15_ENABLED) ===
        if EARLY_C15_ENABLED and c15 and h15 and l15 and v15:
            try:
                broke15, zhi15, zlo15 = detect_compression(h15,l15,c15,v15,zone_bars=6)
            except Exception:
                broke15, zhi15, zlo15 = False,0,0
            if (broke15 and m.get("rsi",100)<=EARLY_RSI_MAX and m["dd"]>KNIFE_DD
                    and m.get("atrr",1.0)>=ATR_MIN_RATIO and btc_hits<BTC_RISK_MIN_HITS
                    and abs(ex.get("funding",0))<FUNDING_CUTOFF
                    and ex.get("spread",0)<SPREAD_MAX*2
                    and m["turn"]>=THIN_TURN):
                lec=LAST_EARLY_C15.get(coin,0)
                if now-lec>=EARLY_C15_COOLDOWN_H*3600:
                    LAST_EARLY_C15[coin]=now
                    bt15=[[{"text":"\u2705 Пробный вход (не подтверждено)","callback_data":f"enter|{coin}|{m['price']:.6g}|trial"}]]
                    tg_send(cid, card_early(m,zhi15,zlo15,tf_label=" 15м",sig_type="early_c15"), buttons=bt15); shown+=1
                    log_signal(coin, "early_c15", m["price"])
        ...
        if w.get("kind")=="reversal_pending":
            ...
            if close_price>w["trigger_price"]:       # ПОДТВЕРЖДЕНО закрытием выше
                ...
                tg_send(chat, card_reversal(mm, w["ex_snapshot"], w["details"]),
                    buttons=[[{"text":"\u2705 Пробный вход (риск повышен)","callback_data":f"enter|{coin}|{close_price:.6g}|trial"}]])
...
STATE_BUCKETS = {
    ...
    "last_alert": LAST_ALERT, "last_early": LAST_EARLY, "last_early_c15": LAST_EARLY_C15,
    "last_early15": LAST_EARLY15, "last_reversal": LAST_REVERSAL, "recent_losses": RECENT_LOSSES,
}
...
def handle_callback(q):
    ...
    if parts[0]=="enter" and len(parts)>=3:
        ...
        tier=parts[3] if len(parts)>=4 else "confirmed"   # "trial" у early/reversal-кнопок, иначе подтверждённый
        ...
        POSITIONS[coin]=dict(entry=price,ts=dt.datetime.now().isoformat(timespec="seconds"),
            sym=sym,last_upd=0,last_check=0,last_state="ok",cor=cor_val,risk0=risk0,tier=tier)
        flush_state()
        tier_note = "\n\U0001F536 Помечено как ПРОБНЫЙ вход (early/reversal) — это будет видно в /pos и в апдейтах." if tier=="trial" else ""
        tg_send(cid,f"\u2705 Веду позицию {coin} от ${price:.5g}.{tier_note}\n"
        ...
        try:
            for u in tg("getUpdates",offset=offset,timeout=30).get("result",[]):
                ...
                if text.startswith("/start"):
                    ...
                    if POSITIONS:
                        parts_pos=[f"{c}\U0001F536" if p.get("tier")=="trial" else c for c,p in POSITIONS.items()]
                        tg_send(cid,"Открытые: "+", ".join(parts_pos)+
                            ("\n\U0001F536 = пробный вход (early/reversal)" if any(p.get('tier')=='trial' for p in POSITIONS.values()) else ""))
