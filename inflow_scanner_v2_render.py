...
СТРАТЕГИЯ (чек-лист из спеки, LONG):
 ...
 12. ЛИМИТЫ: 1 позиция одновременно, 2 сделки в день (сброс 00:00 UTC)
...
def _default_state():
    return dict(day=str(dt.datetime.now(dt.timezone.utc).date()),
                ...
                pending=None,   # {sym, entry, sl, tp1, born_bar, details...}
                position=None)  # {sym, entry, sl, tp1, qty, half_closed, peak, opened}
...
def roll_day(st, chat=None):
    ...
    if d != st.get("day"):
        ...
        save_state(st)
        if chat: tg_send(chat, "\U0001F305 Новый день (UTC) — счётчик сделок обнулён (0/%d)." % MAX_DAILY_TRADES)

def slot_busy(st):
    return st.get("pending") is not None or st.get("position") is not None

def trading_allowed(st):
    if st.get("paused"): return False, "пауза"
    if st.get("trades_today", 0) >= MAX_DAILY_TRADES:
        return False, f"лимит {MAX_DAILY_TRADES}/день исчерпан"
    if slot_busy(st): return False, "слот занят"
    return True, ""
...
def open_pending(st, sym, d, chat):
    st["pending"] = dict(sym=sym, entry=d["entry"], sl=d["sl"], tp1=d["tp1"],
    ...
    L = [
        ...
        f"\U0001F9EA PAPER-режим: сделка виртуальная, учёт честный. Слот: {st.get('trades_today',0)}/{MAX_DAILY_TRADES} сделок сегодня.",
        "Команды: /pos \u00b7 /stats \u00b7 /pause \u00b7 /help",
    ]
    tg_send(chat, "\n".join(x for x in L if x is not None))
    log_signal(sym, d["close3"])

def cancel_pending(st, chat, reason):
    p = st.get("pending")
    if not p: return
    tg_send(chat, f"\u274C {p['sym']}: лимитка отменена — {reason}.")
    st["pending"] = None; save_state(st)

def fill_pending(st, chat, price):
    p = st["pending"]
    qty = NOTIONAL / p["entry"]
    fee_in = NOTIONAL * FEE_MAKER
    st["position"] = dict(sym=p["sym"], entry=p["entry"], sl=p["sl"], tp1=p["tp1"],
    ...
    tg_send(chat,
        ...
        f"\U0001F4C5 Сделка {st['trades_today']}/{MAX_DAILY_TRADES} за день \u00b7 PAPER (комиссии учтены)\n"
        f"Команды: /pos \u00b7 /stats")

def close_part(st, chat, price, part, reason):
    """part: 0.5 или 1.0 от ТЕКУЩЕГО остатка."""
    pos = st["position"]
    ...
    if pos["qty"] <= 1e-12 or part >= 0.999:
        st["position"] = None
        tg_send(chat, "\U0001F4CB Позиция закрыта полностью. Слот свободен "
                      f"({st['trades_today']}/{MAX_DAILY_TRADES} сделок сегодня).")
    save_state(st)

def manage_position(st, chat):
    """Управление по живой цене Bybit: SL / TP1(50%) / трейлинг 0.4%."""
    pos = st.get("position")
    if not pos: return
    price = bybit_price(pos["sym"])
    if price is None: return
    if not pos["half_closed"]:
        if price <= pos["sl"]:
            close_part(st, chat, pos["sl"], 1.0, "СТОП-ЛОСС"); return
        if price >= pos["tp1"]:
            close_part(st, chat, pos["tp1"], 0.5, "ТЕЙК-ПРОФИТ 1 (1:1)")
            pos = st.get("position")
            if pos:
                pos["half_closed"] = True; pos["peak"] = price; save_state(st)
                tg_send(chat, f"\U0001F513 Трейлинг включён: откат {TRAIL_CALLBACK*100:.1f}% от пика. "
                              f"Остаток 50% в безопасности (стоп уже в плюсе).")
            return
    else:
        if price > pos["peak"]:
        ...
        if price <= trail_stop:
            close_part(st, chat, trail_stop, 1.0, "ТРЕЙЛИНГ-СТОП")

def check_pending(st, chat):
    """Проверка ретеста по закрытой 15м-свече Binance: филл / отмена / TTL."""
    p = st.get("pending")
    if not p: return
    try:
        o, h, l, c, v, tb, ct = klines15(p["sym"], limit=3)
    except Exception:
        return
    # последняя ЗАКРЫТАЯ свеча = индекс -2 (последняя может быть незакрытой)
    lo, cl = l[-2], c[-2]
    if cl < p["sl"]:
        cancel_pending(st, chat, "закрытие ниже стопа до входа (структура сломана)"); return
    if lo <= p["entry"]:
        fill_pending(st, chat, p["entry"]); return
    p["ttl"] -= 1
    if p["ttl"] <= 0:
        cancel_pending(st, chat, f"ретеста не было за {ENTRY_TTL_BARS} свечей")
    else:
        save_state(st)
...
def scan_once(st, chat):
    """Проверка закрытых 15м-свечей по вселенной: ищем сетап по чек-листу."""
    ...
    for sym in universe():
        ...
        return  # 1 слот — выставили и вышли
            ...
            for u in r["result"]:
                ...
                if text.startswith("/help"):
                    ...
                    tg_send(cid, "\U0001F916 <b>EVA v3 — импульсный бот (PAPER)</b>\n"
                        ...
                        f"Лимиты: {MAX_CONCURRENT} позиция, {MAX_DAILY_TRADES}/день \u00b7 "
                    ...
                    p, pos = st.get("pending"), st.get("position")
                    if pos:
                        pr = bybit_price(pos["sym"]) or pos["entry"]
                        upnl = (pr - pos["entry"]) * pos["qty"]
                        stage = "трейлинг" if pos["half_closed"] else "ждёт TP1/SL"
                        tg_send(cid, f"\U0001F4CC {pos['sym']}: вход ${pos['entry']:.6g}, сейчас ${pr:.6g} "
                                     f"({upnl:+.2f}$) \u00b7 {stage}\nСделок сегодня: {st['trades_today']}/{MAX_DAILY_TRADES}")
                    elif p:
                        tg_send(cid, f"\u23F3 {p['sym']}: жду ретеста ${p['entry']:.6g} (TTL {p['ttl']} свечей)")
                    else:
                        tg_send(cid, f"Слот свободен. Сделок сегодня: {st.get('trades_today',0)}/{MAX_DAILY_TRADES}")
