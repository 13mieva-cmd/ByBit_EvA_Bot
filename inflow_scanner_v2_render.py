...
TF = "15m"
VOL_MA_LEN = 20
VOL_SPIKE_MIN = 1.5     # ужесточено: без входов на слабом/фейковом объёмном всплеске
...
FIB_RETRACE = 0.385      # 0.0: лимитка-ретест на Фибо больше не используется (плохие исполнения на затухающих пампах)
...
def open_market_position(st, sym, d, chat):
    ...
    L = [
        ...
        f"\U0001F3AF Качество сигнала: {d.get('score',0)}/10 \u00b7 " + " · ".join(d.get("reasons", [])),
        ...
        f"\U0001F3AF TP2: ${d['tp2']:.6g} (справочно, entry + {ATR_TP2_MULT}\u00d7ATR) \u2192 если бы: {tp2_full:+.2f}$ суммарно",
...
def manage_position(st, chat):
    """Частичная фиксация: TP1 (entry+2*ATR) закрывает 50%, SL остатка -> БУ.
    Остаток ведёт ТРЕЙЛИНГ 1.5*ATR от пика (без TP2 - убран как маркетинговая цель:
    при трейлинге 1.5*ATR пройти ещё 2.5*ATR без отката почти никогда не удаётся)."""
    for sym, pos in list(st.get("positions", {}).items()):
        ...
        if not pos["half_closed"]:
            ...
            # Сначала проверяем стопы (БУ и трейлинг), потом обновляем пик — безопаснее.
            if price <= pos["sl"]:
                close_part(st, chat, pos, pos["sl"], 1.0, "СТОП В БУ"); continue
            trail_stop = pos["peak"] - ATR_TRAIL_MULT * pos["atr"]
            if price <= trail_stop:
                close_part(st, chat, pos, trail_stop, 1.0, "ТРЕЙЛИНГ-СТОП (1.5\u00d7ATR)"); continue
            if price > pos["peak"]:
...
def bt_simulate_coin(sym, o, h, l, c, v, tb, ct, oi_ts, diag=None):
    ...
    for i in range(W, len(c)):
        ...
        if pos:
            ...
            if pos and pos["half"]:
                # ПЕССИМИЗМ: сначала стоп/трейл (внутри бара порядок цен неизвестен), потом обновляем пик.
                if bar_l <= pos["sl"]:
                    _bt_leg(pos, pos["sl"], 1.0)
                    pos["close_ts"] = ct[i]; positions.append(pos); pos = None
                    continue
        ...
        if pos is None:
            ...
            if ok:
                ...
                qty = NOTIONAL / d["entry"]
                pos = dict(sym=sym, entry=d["entry"], sl=d["sl"], tp1=d["tp1"],
