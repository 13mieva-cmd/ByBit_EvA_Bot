...
"""
import os, time, json, csv, sqlite3
...
def archive_snapshot(m, ex, passed):
    ...
    try:
        ...
        with open(HISTORY_FILE,"a",newline="") as f:
            ...
            if new: w.writerow(["ts","coin","price","oi1","oi4","oi24","spike","spike_fast",
                "rsi","atrr","cor","p4","turn","funding","spread","dd","uptrend","long_ok"])
            w.writerow([dt.datetime.now().isoformat(timespec="seconds"), m["coin"], f"{m['price']:.8g}",
                ...
                int(bool(m['uptrend'])), int(bool(passed))])
    except Exception as e:
...
def run_scan(cid, announce=False):
    ...
    for coin,sym in coins:
        ...
        by=bybit_price(coin)
        if by: m["bybit"]=by
        archive_snapshot(m, ex, long_ok(m))
...
STATE_BUCKETS = {
    "positions": POSITIONS, "watch": WATCH, "early_watch": EARLY_WATCH,
