...
BODY_RATIO_MAX   = 3.0       # самое крупное тело <= 3x самого мелкого (соразмерность)
...
TRAIL_CALLBACK   = 0.009     # трейлинг-откат 0.4% (по цене Bybit)
FEE_MAKER        = 0.0002    # вход лимиткой (maker) 0.02%
FEE_TAKER        = 0.00055   # выходы по рынку (taker) 0.055% — честный учёт издержек

# --- вселенная ---
MAX_COINS        = 300          # потолок поднят: иначе порог 5M ничего не менял бы (узким местом был топ-120)       # топ по обороту Binance, пересечённый с Bybit
MIN_QUOTE_VOL24  = 5_000_000    # порог оборота снижен: больше монет в скане (тонкие режут 9 фильтров)  # >= $30M суточного оборота (наш проверенный порог)
SCAN_EVERY_SEC   = 90        # проверять закрытие 15м-свечей раз в минуту
MANAGE_EVERY_SEC = 20   # ведение позиции чаще (было 45с): точнее трейлинг/SL без вебсокета;
                        # нагрузка ~3 req/мин на позицию к публичному тикеру Bybit — в лимитах с запасом        # управление позицией (TP/SL/трейлинг) по цене Bybit
...
def klines15(symbol, limit=200):
    """Binance 15m с кэшем 5 мин (снижает нагрузку на API).
    ...
    try:
        ...
        if cache_key in _klines_cache:
            return _klines_cache[cache_key][0]   # stale лучше, чем crash
        raise
...
try:
    import matplotlib
    matplotlib.use("Agg")            # headless: на сервере нет дисплея, plt.show() зависнет
    import matplotlib.pyplot as plt
    HAS_MPL = True
except Exception:
    HAS_MPL = False                  # без matplotlib бэктест работает, просто без картинки

def tg_photo(chat, path, caption=""):
    """Отправка PNG в Telegram (multipart, stdlib-only)."""
...
def bt_klines(symbol, days):
    """История 15м-свечей Binance постранично (limit 1500/запрос)."""
...
def bt_oi(symbol, days):
    """OI-история 15м (Binance хранит только ~30 дней). [(ts, oi), ...]"""
    need = min(days, 30) * 96
    out = []; end = None
    while len(out) < need:
...
def _bt_leg(pos, price, part):
    """Закрытие части позиции: та же математика комиссий, что в live paper."""
    qty_close = pos["qty"] * part
    gross = (price - pos["entry"]) * qty_close
    fee_exit = price * qty_close * FEE_TAKER
...
def bt_simulate_coin(sym, o, h, l, c, v, tb, ct, oi_ts, diag=None):
    ...
    ПЕССИМИЗМ (внутри 15м-бара порядок цен неизвестен):
      ...
      2) после TP1 трейлинг проверяется УЖЕ НА ЭТОМ ЖЕ баре его low —
         если бар волатильный, остаток закроется по трейлу сразу (заниженная оценка).
    Реальный форвардный результат может быть только ЛУЧШЕ этой оценки по данному пункту."""
    W = LEVEL_LOOKBACK + 40
    ...
    for i in range(W, len(c)):
        ...
        if pos:
            if not pos["half"]:
                if bar_l <= pos["sl"]:                      # пессимистично: стоп первым
            ...
            if bar_c < pending["sl"]:
                pending = None                               # структура сломана до входа
...
def bt_portfolio(all_pos, deposit):
    """Реплей хронологически с учётом MAX_CONCURRENT слотов (как live)."""
    taken = []
...
def run_backtest(chat, days=14, ncoins=30, overrides=None):
    ...
    try:
        ...
        txt = (f"\U0001F9EA <b>БЭКТЕСТ: {days} дн \u00d7 {len(coins)} монет</b>\n"
               ...
               f"\u2501\u2501\u2501\u2501\u2501\u2501\u2501\u2501\n"
               f"\u26A0\uFE0F <i>Комиссии учтены; спред/проскальзывание НЕТ; трейлинг по 15м-барам; "
        ...
        if HAS_MPL:
            try:
                ...
                plt.title(f"EVA v3 · эквити бэктеста ({days} дн, {len(coins)} монет, {n} сделок)")
