...
TF = "15m"
VOL_MA_LEN = 20
VOL_SPIKE_MIN = 2.2     # снижено с 2.5: ловим импульс в зародыше, не на хаях
QUIET_BARS = 8          # строго 6 баров затишья перед импульсом
QUIET_MAX = 1.8        # расширено с 1.8: объём чуть "дышит" в полке
QUIET_ALLOW = 0         # строго: ВСЕ 6 баров обязаны быть ниже порога, без исключений
WICK_MAX = 0.25
ATR_LEN = 14
BAR_ATR_MAX = 2.5       # поднято с 2.5: не боимся истинно сильных импульсов
OI_MIN_GROW = 0.01       # отключено: не рубим спот-пампы, где OI на Bybit отстаёт
RSI_LEN = 14
RSI_MAX = 72.0          # поднято с 75: на сильных пампах RSI летит быстро
CVD_MODE = "all"
LEVEL_LOOKBACK = 96
EMA_FAST, EMA_SLOW = 21, 50

# --- вход/выход (маркет-вход по закрытию свечи, лимитки на ретесте отключены) ---
FIB_RETRACE = 0.236      # 0.0 = вход рыночным ордером сразу на закрытии сигнальной свечи, без ретеста
ENTRY_TTL_BARS = 2      # не используется при маркет-входе (оставлено для совместимости)
FEE_MAKER = 0.0002
FEE_TAKER = 0.00055
...
def bybit_market_long(symbol, qty):
    """Рыночный LONG на весь объём позиции."""
...
def open_market_position(st, sym, d, chat):
    """Маркет-вход сразу на закрытии сигнальной 15м-свечи (FIB_RETRACE=0.0, лимитки-ретеста больше нет)."""
    ...
    L = [
        ...
        f"\U0001F4B5 Вход МАРКЕТОМ по закрытию свечи: ${entry_px:.6g}" + (f" \u00b7 Binance close ${d['close3']:.6g}" if by else ""),
        "",
        ...
        f"\U0001F513 Как только TP1 срабатывает (проверка каждые {MANAGE_EVERY_SEC}с): SL остатка \u2192 БУ немедленно, трейлинг {ATR_TRAIL_MULT}\u00d7ATR от пика до TP2",
        "",
...
def scan_once(st, chat):
    """Тик каждые SCAN_EVERY_SEC (5с), НО сигнал считается ТОЛЬКО на закрытии новой 15м-свечи:
    bar_id = ct[-1] последней ЗАКРЫТОЙ свечи, и вход — сразу маркет-ордер, без ожидания ретеста."""
    if BT_RUNNING["on"]:
        return
    ...
    for sym in universe():
        ...
        bar_id = ct[-1]  # id последней ЗАКРЫТОЙ свечи -> гарантия "оценка только на закрытии"
...
def bt_simulate_coin(sym, o, h, l, c, v, tb, ct, oi_ts, diag=None):
    """Симуляция маркет-входа на закрытии сигнальной свечи (FIB_RETRACE=0.0, без ретеста):
