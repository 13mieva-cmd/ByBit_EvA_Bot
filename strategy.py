...
Фаза 1 (detect_fire): голосование + анти-чейз фильтры + HTF-фильтр. Если сошлось —
сетап АРМИРУЕТСЯ (ставится на ожидание отката), НЕ исполняется сразу.
Фаза 2 (check_pullback): на каждом следующем закрытом баре проверяем откат к EMA
и реклейм с объёмом. Только тогда реальный сигнал на исполнение.
"""
import logging
